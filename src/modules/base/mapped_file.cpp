	...
	path_{path}
{
	...
	if (!exists(this->path()))
		throw base::path_not_found{this->path().string()};
	
	if (is_directory(this->path()))
		throw base::path_is_directory{this->path().string()};
	
	if (!is_empty(this->path())) {
		this->file_mapping_  = file_mapping{this->path().c_str(), mode};
		this->mapped_region_ = mapped_region{this->file_mapping_, mode, 0, 0, nullptr, map_options};
	}
}
